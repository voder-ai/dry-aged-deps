...
> Last reviewed: 2026-05-18 — P011 upstream report filed as comment on `windyroad/agent-plugins#126` (Step 5c dedup match — uncertain verdict on same deferred-README seam, different layer; user direction: comment with hook-layer evidence). P012 upstream report pending. Earlier (2026-05-17): retro captured P012 (`@windyroad/itil`'s `migrate-problems-layout.sh` helper uses bashisms but `/wr-itil:work-problems` Step 0a SKILL.md doesn't specify bash — sourcing fails under zsh; recovered via `bash -c` wrap). Step 0a auto-migrated 11 tickets from flat to per-state subdir layout (commit `f44c661`). P011 captured + P010 + P006 upstream reports filed (`windyroad/agent-plugins#137` + `#138`); v2.8.0 released. Dev-work queue top: P006 (WSJF 3.0). Verification Queue: 1 (P009). Parked: 3 (P004, P005, P007).
