...
- [x] **Confirm substance of the proposed ADR-0018 amendment**: the three-class unfixable-reason taxonomy `(a) fix-via-parent-bump` / `(b) fix-via-overrides-edit` / `(c) genuinely-unfixable`. (2026-06-04 user direction confirmed the three-class shape; 2026-06-05 amendment drafted in ADR-0018 with architect + JTBD review APPROVED — see 2026-06-05 Findings below.)
- [x] **Extend the reason taxonomy to a fourth class `(0) fix-via-lockfile-refresh`** (2026-07-08): SHIPPED. `deriveReason()` now classifies a non-bundled vuln with `fixAvailable === true` as `fix via lockfile refresh` (precedence (a) > (0) > (b) > (c)); (b) tightened to the `fixAvailable`-object case. ADR-0018 amended (2026-07-08 amendment: class (0), revised #13/#15/#16, new #18). Covered by `src/find-unfixable-vulns.test.js` class-(0) test + the updated class-(b) object fixture; the RFC-001 gap-#2 boundary test flipped from "mislabel preserved" to "mislabel fixed". Ships via `fix(unfixable-vulns)`.
...
### Findings (2026-06-05 AFK iter 3 — amendment drafted)

- **ADR-0018 amendment landed in-file** at `docs/decisions/0018-surface-known-vulnerable-but-unfixable-packages.proposed.md` under section `## 2026-06-05 Amendment — three-class unfixable-reason taxonomy`. The amendment encodes the user-confirmed three-class taxonomy, the detection signals (parent-walk for class (a); overrides-block parse for class (b); `fixAvailable` interpretation for class (c)), the user-facing reason strings, four additive Confirmation criteria (#13–#17), and the lifecycle decision.
- **Architect review: APPROVED-WITH-AMENDMENTS.** Two substance gaps surfaced and folded in before commit: (i) explicit class precedence ((a) > (b) > (c)) when an advisory could be resolved by either parent-bump or overrides-edit; (ii) multi-advisory packages with mixed classes render the most-actionable class in the table `Reason` cell while JSON/XML retain per-advisory granularity. Architect also flagged that no consumer is known to switch on the retired `vulnerable transitive dependency` literal — noted in the amendment's Backwards compatibility paragraph.
- **JTBD review: APPROVED.** Existing mapping (JTBD-005/006/008/100/202/203) preserved. JTBD-009 ("See unfixable vulnerabilities") was promoted from "anticipated" to "driven by" — the agent flagged that the JTBD has been authored and ratified since the original ADR-0018 draft (`human-oversight: confirmed`, 2026-06-02). Ratification gate clears.
- **Lifecycle decision: L2 (keep `proposed` until code lands).** Architect rationale: an `accepted` ADR whose Confirmation criteria #13–#17 describe behaviour absent from `src/find-unfixable-vulns.js` would assert reality that does not hold. Transition `proposed → accepted` after the TDD + classifier-extension iters complete and Confirmation #14 is observable on this repo.
- **No code changes this iter.** Drafting + review only. The next iter starts the TDD cycle (failing test for class (a) `fix via parent bump`, then implementation, then classes (b) and (c) under their own iters).

### Findings (2026-07-08 — fourth taxonomy class surfaced: `fix-via-lockfile-refresh`)

While recovering a broken CI pipeline on this repo, `find-unfixable-vulns`' remedy taxonomy was observed to point at a **harder remedy than needed** for a whole class of vulns. `deriveReason()` classifies remedy from `npm audit`'s `nodes[]` topology alone — it does NOT check whether a **non-vulnerable version already satisfies the installed semver ranges**. When it does, the real fix is a plain lockfile refresh (`npm audit fix` / incremental `npm install`) with **zero `package.json` edits** — strictly lower-effort than class (a) parent-bump or class (b) overrides-edit.

- **Live case (2026-07-08)**: `sigstore` / `undici` / `vite` were topology-classifiable as `fix via parent bump: npm` / `fix via overrides edit`, but all three were cleared by `npm audit fix` alone — the vulns came from a **stale `package-lock.json`** pinning old versions, not from a range that needed widening. The tool's advice would have sent the user to edit overrides / bump a parent when a lockfile refresh sufficed.
- **Proposed extension**: add a fourth class **(0) `fix-via-lockfile-refresh`** at the TOP of the class-precedence order (lowest-effort remedy, ahead of parent-bump/overrides-edit) — detect when a patched version already satisfies the installed range and surface "refresh your lockfile (`npm audit fix`)" as the preferred fix. This is an ADR-0018 taxonomy amendment (extends the 2026-06-05 three-class amendment).
- **Provenance**: originally observed as a standalone problem on 2026-07-08 and routed here by the `/wr-itil:capture-problem` hang-off-check (verdict HANG_OFF: P013 — this IS the `deriveReason()` reason taxonomy P013 gap #2 owns). Note the three-class taxonomy itself has since shipped — class (c) landed 2026-07-08 in the `fix(unfixable-vulns)` commit — so this fourth class is additive scope on top of the shipped classifier, not part of the original unshipped taxonomy.
