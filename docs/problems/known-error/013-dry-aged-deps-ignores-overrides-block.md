...
- [ ] **Extend the reason taxonomy to a fourth class `(0) fix-via-lockfile-refresh`** (2026-07-08): detect when a non-vulnerable version already satisfies the installed semver range (fixable by `npm audit fix` / incremental `npm install` alone, no `package.json` edit) and surface it at the TOP of the class-precedence order, ahead of parent-bump / overrides-edit. Reproduction fixture: the 2026-07-08 sigstore/undici/vite stale-lockfile case (see Findings below).
