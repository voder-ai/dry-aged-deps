...
superseded-by: '0020-test-placement-co-locate-with-src-module'
---

# 0015. Test placement: co-location exception for `src/update-packages.test.js`

> **Superseded by [ADR-0020](./0020-test-placement-co-locate-with-src-module.proposed.md)** (2026-05-30, same day). ADR-0015's narrow-exception framing (Option 1 of its considered options) is replaced by ADR-0020's universal co-location convention (originally rejected as ADR-0015 Option 3). The third-trigger forcing function this ADR documented (§Reassessment 2 + §Consequences/Bad) fired empirically in RFC-001 T4 (iter 4 of the 2026-05-30 `/wr-itil:work-problems` AFK loop), prompting the user to adopt the previously-rejected Option 3 as the new convention. This file is preserved for lineage and historical context; ADR-0020 is the current authority on test placement.
