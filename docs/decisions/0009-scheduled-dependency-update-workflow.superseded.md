...
- **Do not allow bypassing the above settings** — leave **DISABLED** (this is the GitHub default). When unchecked, repository administrators (which the repo owner — the maintainer — is by default) bypass the required-status-checks rule on direct pushes. This is the mechanism that preserves TBD: the maintainer's direct push to `main` succeeds even though the pushed SHA's `Build & Test` check has not yet been recorded as passing (the check runs _after_ the push lands, on the resulting `push`-event); the bot's PR is not authored by an admin so the rule is enforced and `gh pr merge --auto --squash` waits for green.

If, in future, additional collaborators are added without admin role, their direct pushes to `main` would be blocked by the required-status-checks rule (they have no admin bypass). For that scenario, switch to GitHub Rulesets and scope the rule using an explicit bypass list. See Reassessment Criterion 3 for the trigger condition.
