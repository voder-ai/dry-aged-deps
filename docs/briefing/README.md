...
- **Push goes through `npm run push:watch`.** A global Claude PreToolUse hook intercepts `git push` and redirects. The wrapper lives at `scripts/push-watch.sh` and aborts pre-emptively if stale safe deps are detected. Post-push release-trigger heuristic captures `@{push}` BEFORE push (P002 verifying, shipped v2.7.1).
