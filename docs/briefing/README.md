...
- **Pre-commit hook is read-only (ADR-0013, P003 verifying).** Husky pre-commit runs `npm run format:check` (no auto-write). On failure: abort with guidance to run `npm run format` and re-stage. Workspace state dirs (`.risk-reports/`, `.afk-run-state/`, `.claude/`, `*.backup`) are in `.prettierignore` so the check scopes to source-controlled files only.
