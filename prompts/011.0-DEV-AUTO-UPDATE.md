...
- [ ] **Dry Run**: Shows what would be updated before making changes
- [ ] **Confirmation**: Requires confirmation before updating (unless `--yes` flag provided)
...
- **REQ-DRY-RUN**: Default mode shows what would be updated (read-only)
...
- **REQ-CONFIRMATION**: Interactive confirmation unless `--yes` provided
...
```bash
dry-aged-deps --update --dry-run
# Shows what would be updated but doesn't modify package.json
```
