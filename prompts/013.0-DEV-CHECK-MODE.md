...
- [ ] **Threshold Option**: Optional `--max-age` to fail only if updates are very old
...
- **REQ-OPTIONAL-THRESHOLD**: Support `--max-age` to fail only on old updates (optional enhancement)
...
```bash
# Only fail if updates are VERY old (> 90 days behind)
dry-aged-deps --check --max-age=90
# Exit 1 only if safe updates are more than 90 days old
```
...
- [ ] Tests for threshold behavior (if implemented)
...
**Flexible Policies**: Basic check mode fails on any safe update. Optional `--max-age` threshold allows more nuanced policies (e.g., "fail only if safe updates are >30 days old").

**CI/CD Philosophy**: Some teams want CI to warn about updates (exit 1 = informational). Others want CI to fail until updates are applied (exit 1 = failure). Check mode enables the second approach.
...
**Future Enhancement**: Could add `--check-severity` to fail only on certain vulnerability severities, or `--check-prod` to enforce policies only for production dependencies.
