...
describe('Story 004.0-DEV-FILTER-VULNERABLE-VERSIONS: computeVulnerabilityStats', () => {
  const severityWeights = { none: 0, low: 1, moderate: 2, high: 3, critical: 4 };

  it('[REQ-SAFE-ONLY] returns correct metrics when details array present', () => {
    const result = {
      count: 3,
      details: [
        { id: 'a', severity: 'low' },
  ...
  it('[REQ-SAFE-ONLY] infers count from details length when count missing', () => {
    const details = [
  ...
  it('[REQ-SAFE-ONLY] handles missing details array', () => {
  ...
  it('[REQ-SAFE-ONLY] defaults weight to 0 for unknown severity labels', () => {
...
describe('Story 004.0-DEV-FILTER-VULNERABLE-VERSIONS: countAboveThreshold', () => {
  const severityWeights = { none: 0, low: 1, moderate: 2, high: 3, critical: 4 };

  it('[REQ-SAFE-ONLY] counts vulnerabilities meeting threshold', () => {
    const list = [{ severity: 'low' }, { severity: 'moderate' }, { severity: 'high' }];
    expect(countAboveThreshold(list, 2, severityWeights)).toBe(2);
  });

  it('[REQ-SAFE-ONLY] includes vulnerabilities equal to threshold', () => {
    const list = [{ severity: 'moderate' }, { severity: 'high' }];
    expect(countAboveThreshold(list, 2, severityWeights)).toBe(2);
  });

  it('[REQ-SAFE-ONLY] returns 0 when no details meet threshold', () => {
    const list = [{ severity: 'low' }];
    expect(countAboveThreshold(list, 2, severityWeights)).toBe(0);
  });

  it('[REQ-SAFE-ONLY] handles empty details list', () => {
    expect(countAboveThreshold([], 1, severityWeights)).toBe(0);
  });

  it('[REQ-SAFE-ONLY] handles severity missing in object', () => {
    const list = [{}, { severity: '' }];
